...
import { faGithub } from "@fortawesome/free-brands-svg-icons"; // GitHub icon import
...
const Navbar = () => {
  ...
  return (
    <div className="text-gray-500 dark:text-gray-300 flex  justify-between items-center h-24 max-w-[1300px] mx-auto px-2">
      ...
      {/* Mobile Navigation Icon (hamburger / close) */}
<div className="block md:hidden">
  ...
  >
    {nav ? (
      ...
      <ul
  className={`fixed md:hidden left-0 top-0 w-full h-full overflow-y-auto z-40 
    bg-[#000300] dark:bg-gray-900 transition-all duration-500 ease-in-out ${
      nav ? "left-0" : "left-[-100%]"
    }`}
>
  {/* Logo/Title stays at top */}
    ...
    {navItems.map((item: NavItem) => (
      <li
        ...
        className="p-4 border-b border-gray-600 rounded-xl 
          ...
          transition duration-300 text-gray-300 w-full max-w-md text-center"
      >
        {item.text}
      </li>
    ))}
  ...
  <li className="p-4 mt-4 hidden md:block   ">
    ...
    https://github.com/apelmahmudDev/e-furniture
  </li>
</ul>
    </div> 
  );
